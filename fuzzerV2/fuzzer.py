...
_FUZZER_DIR = Path(__file__).resolve().parent.parent / "fuzzer"
...
def list_scenario_files(subdir):
    target_dir = (_FUZZER_DIR / subdir).resolve()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Browser Fuzzer V2 (Selenium)")
    parser.add_argument(
        "-r", "--report-url",
        type=str,
