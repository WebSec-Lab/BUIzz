...
from .core_common import (
    ...
    get_browser_exe, make_chromium_service, force_window_maximize,
    reposition_if_offscreen, _SCENARIO_TIMEOUT,
)

_PROCESS_NAME     = "brave.exe"
_RESTART_SLEEP    = 4
...
def _fresh_session(browser_name):
    app, driver, browser_x, browser_y = _start_browser(browser_name)
    reset_to_single_tab(driver)
    driver.execute_script("window.focus()")
    time.sleep(0.5)
    force_window_maximize(app)
    browser_x, browser_y = check_browserx_browsery(app)
    print(f"[debug] Brave window origin: ({browser_x}, {browser_y})")
    return app, driver, browser_x, browser_y
